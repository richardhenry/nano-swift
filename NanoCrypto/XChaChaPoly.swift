...
/// An implementation of the XChaCha20-Poly1305 extended nonce cipher suitable for use with random nonces.
public enum XChaChaPoly {
    ...
    public static let tagBytes = Int(crypto_aead_xchacha20poly1305_IETF_ABYTES)
    public static let nonceBytes = Int(crypto_aead_xchacha20poly1305_IETF_NPUBBYTES)
}

extension XChaChaPoly {
    ...
    public struct SealedBox {
        ...
        public var tag: Data {
            combined.suffix(XChaChaPoly.tagBytes)
        }

        /// The ciphertext.
        public var ciphertext: Data {
            combined.dropFirst(XChaChaPoly.nonceBytes).dropLast(XChaChaPoly.tagBytes)
        }

        /// The nonce used to encrypt the data.
        public var nonce: Nonce {
            try! Nonce(data: combined.prefix(XChaChaPoly.nonceBytes))
        }
        ...
        public init<D: DataProtocol>(combined: D) throws {
            guard combined.count >= XChaChaPoly.nonceBytes + XChaChaPoly.tagBytes else {
...
extension XChaChaPoly {
    ...
    public struct Nonce {
        ...
        /// Creates a new random 24 byte nonce.
        public init() {
            data = Data(randomBytes: XChaChaPoly.nonceBytes)
        }

        /// Creates a nonce from the given data or throws if the size is incorrect.
        public init<D: DataProtocol>(data: D) throws {
            guard data.count == XChaChaPoly.nonceBytes else {
...
extension XChaChaPoly {
    ...
    static func sealContiguous(
        ...
        let result = crypto_aead_xchacha20poly1305_ietf_encrypt_detached(
...
extension XChaChaPoly {
    ...
    static func openContiguous(
        ...
        let result = crypto_aead_xchacha20poly1305_ietf_decrypt_detached(
