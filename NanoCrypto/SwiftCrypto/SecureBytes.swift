...
struct SecureBytes {
    ...
    init(randomBytes: Int) {
        if randomBytes == 0 {
            ...
            self.backing = SecureBytes.Backing.create(randomBytes: randomBytes)
        }
    }
    ...
    init(zeroBytes: Int) {
        if zeroBytes == 0 {
            ...
            self.backing = SecureBytes.Backing.create(zeroBytes: zeroBytes)
        }
    }
...
extension SecureBytes: RangeReplaceableCollection {
    ...
    @inlinable
    public mutating func append<Elements: Sequence>(contentsOf newElements: Elements)
    where Elements.Element == UInt8 {
...
extension SecureBytes: ContiguousBytes {
    ...
    @inlinable
    mutating func withUnsafeMutableBytes<T>(
    ...
    internal class Backing: ManagedBuffer<BackingHeader, UInt8> {
        ...
        class func createEmpty() -> Backing {
            return Backing.create(
                minimumCapacity: 0,
                makingHeaderWith: { _ in BackingHeader(count: 0, capacity: 0) }
            )
                as! Backing
        }
        ...
        @usableFromInline
        class func create(zeroBytes: Int) -> Backing {
            let backing = Backing.create(capacity: zeroBytes)
            backing._withVeryUnsafeMutableBytes { targetPtr in
                assert(targetPtr.count >= zeroBytes)
                memset_s(targetPtr.baseAddress!, zeroBytes, 0, zeroBytes)
            }
            backing.count = zeroBytes
            return backing
        }
